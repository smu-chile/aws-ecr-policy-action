...
function login() {
  ...
  LOGIN_COMMAND=$(docker run --rm -ti -v ~/.aws:/root/.aws -v $(pwd):/aws amazon/aws-cli:2.0. ecr get-login-password --region $INPUT_REGION | docker login --username AWS --password-stdin $INPUT_ECR_REGISTRY)
  $LOGIN_COMMAND
