...
function login() {
  ...
  LOGIN_COMMAND=$(docker run --rm -i -v ~/.aws:/root/.aws -e AWS_ACCESS_KEY_ID=$INPUT_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$INPUT_SECRET_ACCESS_KEY  amazon/aws-cli ecr get-login-password --region $INPUT_REGION )
